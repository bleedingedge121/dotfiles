...
get_hex() {
    COLORS=$(cat ~/.local/state/quickshell/user/generated/colors.json)
    PRIMARY=$(echo "$COLORS" | jq -r '.primary')
    TERTIARY=$(echo "$COLORS" | jq -r '.tertiary')
    SECONDARY=$(echo "$COLORS" | jq -r '.secondary')
    PRIMARY_FIXED=$(echo "$COLORS" | jq -r '.primary_fixed')

    case $1 in
        "50d"|"50n") echo "$SECONDARY" ;;
        "01d") echo "$PRIMARY_FIXED" ;;
        "01n") echo "$TERTIARY" ;;
        "02d"|"02n"|"03d"|"03n"|"04d"|"04n") echo "$SECONDARY" ;;
        "09d"|"09n"|"10d"|"10n") echo "$TERTIARY" ;;
        "11d"|"11n") echo "$PRIMARY_FIXED" ;;
        "13d"|"13n") echo "$TERTIARY" ;;
        *) echo "$PRIMARY" ;;
    esac
}
